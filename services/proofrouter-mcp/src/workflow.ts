...
import { createMockLlmGateway, DEFAULT_MOCK_MODEL_ID, LlmGateway } from "./llm.js";
...
export async function executeInferenceOrder(
  ...
  const llm = deps.llm ?? createMockLlmGateway(env);
  const offers = listProxyOffers(env);
  const modelId = env.MOCK_LLM_MODEL ?? DEFAULT_MOCK_MODEL_ID;
  ...
  const resultWithoutTrace: OrderResult = {
    ...
    timeline: [
      ...
      {
        label: "Mock LLM response",
        status: "done",
        detail: "Answer generated locally; no OpenAI, LiteLLM, or external LLM provider was called."
      },
      {
